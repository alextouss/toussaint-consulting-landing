...
- **Captures produit / visuels embarqués** : garder les **couleurs d'origine** (ne jamais re-thémer un screenshot produit ou un widget à la palette de marque — ça fait faux). Les présenter comme des **fenêtres claires flottantes** sur les slides dark : cadre `.capture` (coins arrondis + fine bordure claire + ombre douce, `background:#fff`, `box-shadow` rend en print), caption muted au-dessus (hors de la carte dark). Pour un widget thémable, rendre le vrai artefact (flipper le `data-theme` de la preview, sur le `<html>` seulement, pas le sélecteur CSS). Cf. skill `pdf-export` gotcha #12 + memory `feedback_realistic_product_visuals`. Le fond dark pur (`#0A0A0A`) peut être adouci vers un charbon chaud (ex. `#15110D`) si un deck paraît trop sombre.
