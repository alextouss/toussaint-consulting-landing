...
- **Captures produit / visuels embarqués** (règle de build HTML) : garder les **couleurs d'origine** (ne jamais re-thémer un screenshot produit ou un widget à la palette de marque — ça fait faux ; un graphique re-teinté a été rejeté). L'UI produit réelle est généralement claire → les présenter comme des **fenêtres claires flottantes** sur les slides dark. Caption muted **au-dessus** sur le slide (hors d'une carte dark, sinon illisible), image dans un cadre `.capture` :
  ...
  ```
  Pour un widget thémable, rendre le **vrai artefact** (flipper le `data-theme` de la preview, sur le `<html>` seulement, **pas** le sélecteur CSS `[data-theme]` sinon on re-matche la règle dark). Height-cap d'une capture large : cibler l'`img` (`max-height: Nmm; width:auto; margin:0 auto`), elle se centre dans le cadre blanc. Narration dark + captures produit claires = langage SaaS classique. Le `box-shadow` survit en PDF → cf. skill `pdf-export` gotcha #12 (volet print uniquement). Memory `feedback_realistic_product_visuals`. Le fond dark pur (`#0A0A0A`) peut être adouci vers un charbon chaud (ex. `#15110D`) si un deck paraît trop sombre.
  - **Marges des captures (fixer l'asset, pas le CSS)** : un screenshot d'artefact claude.ai est souvent un crop trop serré → contenu collé aux bords, labels de bord rognés. Ne **pas** compenser avec un padding sur `.capture` (le contenu reste collé au bord du raster) → corriger **le PNG lui-même**. Si une **source régénérable** existe (widget thémable), re-rendre avec un body paddé puis screenshot (le `padding` autour du SVG donne la marge ; screenshoter le body, pas le `.dc-wrap`, car les labels remplissent la largeur du SVG). Sinon, **baker une marge blanche** dans le raster (page blanche paddée + re-screenshot — le fond blanc se fond). Helper : `node scripts/regen-benoit-captures.mjs`. Cas Benoit : `demo-chart.png` régénéré depuis `benoit_pagny/ui/dist/preview-dark.html` (données RESTO inlinées) flippé en light ; `demo-portfolio.png` = artefact one-off **sans source** (l'outil MCP ne renvoie qu'un tableau markdown) → marge bakée.
